jQuery(document).ready(function() {
	...
	jQuery.widget( "ui.combobox", {
		_create: function() {
			...
			var input = this.input = jQuery( "<input>" )
				...
				.val( value )
				.attr('size',30)
				.attr('placeholder','Start typing to search for a venue')
				.autocomplete({
					delay: 0,
					minLength: 2,
					...
					select: function(event, ui) {
						if(jQuery("tr.venue_row.novenue").length >0){
							jQuery("tr.venue_row.novenue").show();
							jQuery("#hws_vm_name").html(ui.item.venue_name);
							jQuery("#hws_vm_addr").html(ui.item.venue_address);
							jQuery("#hws_vm_postal").html(ui.item.venue_postCode);
							jQuery("#hws_vm_country").html(ui.item.venue_country);
							initialize(ui.item.venue_lat,ui.item.venue_lng);
						}
							jQuery("#venue_select").removeAttr("selected");
							jQuery("#venue_select").val(ui.item.venue_id);
					}	
				})
	...
	var dates = jQuery("#eventorganiser_detail #from_date, #eventorganiser_detail #to_date" ).datepicker({
			...
			onSelect: function( selectedDate ) {
				...
				if( this.id == "from_date"){
					jQuery( "#recend").datepicker( "option", "minDate", date );
				}
	...
	if(jQuery("#HWSEventInput_Req").val()!='weekly'){
		jQuery("#dayofweekrepeat").hide();
	}	
	if(jQuery("#HWSEventInput_Req").val()!='monthly'){
		jQuery("#dayofmonthrepeat").hide();
	}
	jQuery("tr.venue_row.novenue").hide();
	jQuery(".onetime .reocurrence_row").hide();;


	//When checked, a user wants to edit a reoccurring event.
	jQuery("#HWSEvent_rec").click(function(){
		var bool = !jQuery(this).prop("checked");
		jQuery(".reoccurence .event-date :input").attr('disabled', bool);
	});

	//When any input is altered. Update the form.
	jQuery(".reoccurence .event-date :input, .onetime .event-date :input").change(function(){
		eo_update_event_form();
	});
...
function eo_produce_summary(){
		...
		if(jQuery("#HWSEventInput_Req").val()=='once'){
			jQuery("#event_summary").html('This event will be a one-time event');
			return;
		}
	
		var fromdate = jQuery("#from_date").datepicker("getDate");
		var weekdays=new Array("Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday");
		var ical_weekdays=new Array("SU","MO","TU","WE","TH","FR","SA");

		//Get reoccurrence and frequency
		reoccurrence =jQuery("#HWSEventInput_Req :selected").text();
		...
		switch(jQuery("#HWSEventInput_Req").val()){
			case 'weekly':
				...
				if(selected.length==0){
					day =fromdate.getDay();
					jQuery("#dayofweekrepeat :checkbox[value='"+ical_weekdays[day]+"']").attr('checked',true);
				}
				selected = jQuery("#dayofweekrepeat :checkbox:checked");
		
				selected.each(function(index){
					if(index==0)summary = summary+" on "+weekdays[ical_weekdays.indexOf(jQuery(this).val())];
					if(index>0)summary = summary+", "+weekdays[ical_weekdays.indexOf(jQuery(this).val())];
				});
				break;
		...
		var schedule_end = jQuery("#recend").datepicker("getDate");
		if(schedule_end!= null){			
			summary = summary+" until "+ jQuery.datepicker.formatDate("MM d'"+eo_date_suffix(schedule_end)+"' yy", schedule_end);
		}

		//Display summary
		jQuery("#event_summary").html(summary);		
	};
...
function eo_update_event_form(){
		...
		var bool = !jQuery("#eo_allday:checkbox").attr('checked');
		 jQuery(".eo_time").attr('disabled', !bool);
		...
		*/
		switch(jQuery("#HWSEventInput_Req").val()){
			case 'once':
				...
				jQuery('#HWSEvent_freq').val('1');
				jQuery(".reocurrence_row").hide();
				jQuery("#dayofweekrepeat").show();
				jQuery("#dayofmonthrepeat").show();
				jQuery(".reocurrence_row").attr('disabled', true);
				break;

			case 'weekly':
				//Show & enable reoccurrence forms and week metaa. Disable & hide month meta 
				jQuery(".reocurrence_row :input").attr('disabled', false);
				jQuery("#recpan").text('week');
				jQuery(".reocurrence_row").fadeIn(speed);
				jQuery("#dayofweekrepeat").fadeIn(speed);
				jQuery("#dayofweekrepeat :input").attr('disabled', false);
				jQuery("#dayofmonthrepeat").hide();
				jQuery("#dayofmonthrepeat :radio").attr('disabled', true);
				break;

			case 'monthly':
				//Show & enable reoccurrence forms and month meta. Disable & hide week meta 
				jQuery(".reocurrence_row :input").attr('disabled', false);
				jQuery("#recpan").text('month');
				jQuery(".reocurrence_row").fadeIn(speed);
				jQuery("#dayofmonthrepeat").fadeIn(speed);
				jQuery("#dayofmonthrepeat :input").attr('disabled', false);
				jQuery("#dayofweekrepeat").hide();
				jQuery("#dayofweekrepeat :input").attr('disabled', true);
				break;
		...
		if(jQuery("#HWSEvent_freq").val() >1){
			jQuery("#recpan").text(jQuery("#recpan").text()+'s');
		}

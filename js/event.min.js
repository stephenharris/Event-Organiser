...
m=n.summary+" ";switch(b("#HWSEventInput_Req").val()){case "custom":case "daily":m+=i>1?sprintf(n.dayPlural,i):n.daySingle;break;case "weekly":m+=i>1?sprintf(n.weekPlural,i):n.weekSingle;i=b("#dayofweekrepeat :checkbox:checked");if(i.length==0){i=g.getDay();b("#dayofweekrepeat :checkbox[value='"+u[i]+"']").attr("checked",true)}i=b("#dayofweekrepeat :checkbox:checked");i.each(function(p){if(p==0)m=m+" "+h[u.indexOf(b(this).val())];if(p>0)m=m+", "+h[u.indexOf(b(this).val())]});break;case "monthly":m+=
i>1?sprintf(n.monthPlural,i):n.monthSingle;if(b("#dayofmonthrepeat :radio:checked").val()=="BYMONTHDAY=")m=m+" "+g.getDate()+a(g);else{i=g.getDay()%7;g=parseInt(Math.floor((g.getDate()-1)/7));m=m+" "+n.occurrence[g]+" "+h[i]}break;case "yearly":m+=i>1?sprintf(n.yearPlural,i):n.yearSingle;m=m+" "+b.datepicker.formatDate("MM d",g,o)+a(g);break}g=b("#recend").datepicker("getDate");if(g!=null)m=m+" "+n.until+" "+b.datepicker.formatDate("MM d'"+a(g)+"' yy",g,o);b("#event_summary").html(m)}}function a(g){return["th",
...
o=parseInt(b("#HWSEvent_freq").val()),i=b("#from_date").datepicker("getDate"),m=b("#recend").datepicker("getDate");h-=1;var p=new Date(g,h,1),z=new Date(new Date(g,h+1,1)-1);if(!(m<p||i>z)){switch(u){case "once":case "custom":g=b.datepicker.formatDate("yy-mm-dd",i);s.push(g);return;case "daily":if(i<p){var C=Math.abs((p-i)/864E5)-1;C%=o}else C=parseInt(i.getDate());h=o;var G=[],E=new Date(p);E.setDate(p.getDate()+(C-1));G.push(E);break;case "weekly":var r=p.getDay();r=b("#dayofweekrepeat :checkbox:checked");
var J=new Array("SU","MO","TU","WE","TH","FR","SA");G=[];r.each(function(I){I=J.indexOf(b(this).val());E=new Date(i);E.setDate(i.getDate()+(I-i.getDay()+7)%7);if(E<p){C=Math.abs((p-i)/864E5);C-=C%(o*7);E.setDate(E.getDate()+C)}G.push(E)});h=7*o;break;case "monthly":if(((g-i.getFullYear())*12+h-(i.getMonth()+1))%o!=0)return;if(b('input[name="eo_input[schedule_meta]"]:checked').val()=="BYMONTHDAY="){var F=i.getDate();z=z.getDate();if(F<=z)r=new Date(g,h,F)}else{g=Math.ceil(i.getDate()/7);F=i.getDay();
if(g>=5){r=z.getDay();z=z.getDate()+(F-r-7)%7}else{r=p.getDay();z=(F-r+7)%7+(g-1)*7+1}r=new Date(p);r.setDate(z)}if(r<=m){g=b.datepicker.formatDate("yy-mm-dd",r);s.push(g)}return;case "yearly":if((g-i.getFullYear())%o!=0)return;z=new Date(g,i.getMonth(),i.getDate());if(h==i.getMonth()&&z.getMonth()==i.getMonth()){r=new Date(i);r.setYear(g);if(r<=m){g=b.datepicker.formatDate("yy-mm-dd",r);s.push(g)}}return;default:return}for(F in G)for(r=new Date(G[F]);r<=z&&r<=m;){g=b.datepicker.formatDate("yy-mm-dd",
r);s.push(g);r.setDate(r.getDate()+h)}}}function j(g){var h=d(g);if(h[0]){h=h[1];h>-1?k(h):t(g)}else{h=h[1];h>-1?A(h):l(g)}B()}function l(g){b.inArray(g,w)<0&&w.push(g)}function k(g){w.splice(g,1)}function t(g){b.inArray(g,x)<0&&x.push(g)}function A(g){x.splice(g,1)}if(!Array.prototype.indexOf)Array.prototype.indexOf=function(g,h){var u=this.length;h=Number(h)||0;h=h<0?Math.ceil(h):Math.floor(h);if(h<0)h+=u;for(;h<u;h++)if(h in this&&this[h]===g)return h;return-1};var s=[],w,x;if(b("#eo_occurrence_includes").length>
...
if(A&&A.apply(a.input?a.input[0]:null,[a.hours,a.minutes])==false)for(k=0;k<j.length;k+=1){c=j[k];if(A.apply(a.input?a.input[0]:null,[a.hours,c])){a.minutes=c;break}}d+='<div class="ui-timepicker-title ui-widget-header ui-helper-clearfix ui-corner-all">'+s+'</div><table class="ui-timepicker">';k=0;for(e=1;e<=f;e++){for(d+="<tr>";k<e*l;){c=j[k];A="";if(c!==undefined)A=c<10&&t?"0"+c.toString():c.toString();d+=this._generateHTMLMinuteCell(a,c,A);k++}d+="</tr>"}d+="</table>";return d},_generateHTMLHourCell:function(a,

...
null&&b("tr.venue_row").hide();q()}function q(){if(b("#HWSEventInput_Req").val()=="once")b("#event_summary").html("This event will be a one-time event");else{var g=b("#from_date").datepicker("getDate"),i=EO_Ajax_Event.locale.weekDay,t=new Array("SU","MO","TU","WE","TH","FR","SA"),m={monthNamesShort:EO_Ajax_Event.locale.monthAbbrev,dayNamesMin:EO_Ajax_Event.locale.dayAbbrev,monthNames:EO_Ajax_Event.locale.monthNames};b("#HWSEventInput_Req :selected").text();var h=parseInt(b("#HWSEvent_freq").val()),
n=o.summary+" ";switch(b("#HWSEventInput_Req").val()){case "custom":case "daily":n+=h>1?sprintf(o.dayPlural,h):o.daySingle;break;case "weekly":n+=h>1?sprintf(o.weekPlural,h):o.weekSingle;h=b("#dayofweekrepeat :checkbox:checked");if(h.length==0){h=g.getDay();b("#dayofweekrepeat :checkbox[value='"+t[h]+"']").attr("checked",true)}h=b("#dayofweekrepeat :checkbox:checked");h.each(function(p){if(p==0)n=n+" "+i[t.indexOf(b(this).val())];if(p>0)n=n+", "+i[t.indexOf(b(this).val())]});break;case "monthly":n+=
h>1?sprintf(o.monthPlural,h):o.monthSingle;if(b("#dayofmonthrepeat :radio:checked").val()=="BYMONTHDAY=")n=n+" "+g.getDate()+a(g);else{h=g.getDay()%7;g=parseInt(Math.floor((g.getDate()-1)/7));n=n+" "+o.occurrence[g]+" "+i[h]}break;case "yearly":n+=h>1?sprintf(o.yearPlural,h):o.yearSingle;n=n+" "+b.datepicker.formatDate("MM d",g,m)+a(g);break}g=b("#recend").datepicker("getDate");if(g!=null)n=n+" "+o.until+" "+b.datepicker.formatDate("MM d'"+a(g)+"' yy",g,m);b("#event_summary").html(n)}}function a(g){return["th",
"st","nd","rd"][3<g.getDate()&&g.getDate()<20?0:Math.min(g.getDate()%10,4)%4]}function c(){eo_exclude_dates=[];eo_include_dates=[];z();f(eo_viewing_month[0],eo_viewing_month[1],{});C.datepicker("refresh")}function e(g){g=b.datepicker.formatDate("yy-mm-dd",g);if(d(g)[0])return[true,"ui-state-active",""];return[true,"ui-state-disabled",""]}function d(g){if(b.inArray(g,eo_occurrences_by_rule)>-1){g=b.inArray(g,eo_exclude_dates);return g>-1?[false,g]:[true,-1]}else{g=b.inArray(g,eo_include_dates);return g>
-1?[true,g]:[false,-1]}}function f(g,i){eo_occurrences_by_rule=[];eo_viewing_month=[g,i];var t=b("#HWSEventInput_Req").val(),m=parseInt(b("#HWSEvent_freq").val()),h=b("#from_date").datepicker("getDate"),n=b("#recend").datepicker("getDate");i-=1;var p=new Date(g,i,1),w=new Date(new Date(g,i+1,1)-1);if(!(n<p||h>w)){switch(t){case "once":case "custom":g=b.datepicker.formatDate("yy-mm-dd",h);eo_occurrences_by_rule.push(g);return;case "daily":if(h<p){var A=Math.abs((p-h)/864E5)-1;A%=m}else A=parseInt(h.getDate());
i=m;var E=[],B=new Date(p);B.setDate(p.getDate()+(A-1));E.push(B);break;case "weekly":var r=p.getDay();r=b("#dayofweekrepeat :checkbox:checked");var H=new Array("SU","MO","TU","WE","TH","FR","SA");E=[];r.each(function(G){G=H.indexOf(b(this).val());B=new Date(h);B.setDate(h.getDate()+(G-h.getDay()+7)%7);if(B<p){A=Math.abs((p-h)/864E5);A-=A%(m*7);B.setDate(B.getDate()+A)}E.push(B)});i=7*m;break;case "monthly":if(((g-h.getFullYear())*12+i-(h.getMonth()+1))%m!=0)return;if(b('input[name="eo_input[schedule_meta]"]:checked').val()==
"BYMONTHDAY="){var F=h.getDate();w=w.getDate();if(F<=w)r=new Date(g,i,F)}else{g=Math.ceil(h.getDate()/7);F=h.getDay();if(g>=5){r=w.getDay();w=w.getDate()+(F-r-7)%7}else{r=p.getDay();w=(F-r+7)%7+(g-1)*7+1}r=new Date(p);r.setDate(w)}if(r<=n){g=b.datepicker.formatDate("yy-mm-dd",r);eo_occurrences_by_rule.push(g)}return;case "yearly":if((g-h.getFullYear())%m!=0)return;w=new Date(g,h.getMonth(),h.getDate());if(i==h.getMonth()&&w.getMonth()==h.getMonth()){r=new Date(h);r.setYear(g);if(r<=n){g=b.datepicker.formatDate("yy-mm-dd",
r);eo_occurrences_by_rule.push(g)}}return;default:return}for(F in E)for(r=new Date(E[F]);r<=w&&r<=n;){g=b.datepicker.formatDate("yy-mm-dd",r);eo_occurrences_by_rule.push(g);r.setDate(r.getDate()+i)}}}function j(g){var i=d(g);if(i[0]){i=i[1];i>-1?k(i):s(g)}else{i=i[1];i>-1?x(i):l(g)}z()}function l(g){b.inArray(g,eo_include_dates)<0&&eo_include_dates.push(g)}function k(g){eo_include_dates.splice(g,1)}function s(g){b.inArray(g,eo_exclude_dates)<0&&eo_exclude_dates.push(g)}function x(g){eo_exclude_dates.splice(g,
1)}if(!Array.prototype.indexOf)Array.prototype.indexOf=function(g,i){var t=this.length;i=Number(i)||0;i=i<0?Math.ceil(i):Math.floor(i);if(i<0)i+=t;for(;i<t;i++)if(i in this&&this[i]===g)return i;return-1};var u;if(b("#eo_occurrence_includes").length>0){eo_include_dates=b("#eo_occurrence_includes").val().split(",");eo_exclude_dates=b("#eo_occurrence_excludes").val().split(",")}b(document).ready(function(){b.widget("ui.combobox",{_create:function(){var g,i=this.element.hide(),t=i.children(":selected");
t=t.val()?t.text():"";i=b("<span>").addClass("ui-combobox eo-venue-input").insertAfter(i);g=b("<input>").appendTo(i).val(t).addClass("ui-combobox-input").autocomplete({delay:0,minLength:0,source:function(m,h){b.getJSON(EO_Ajax_Event.ajaxurl+"?callback=?&action=eo-search-venue",m,function(n){n=b.map(n,function(p){p.label=p.name;return p});h(n)})},select:function(m,h){if(b("tr.venue_row").length>0){h.item.term_id==0?b("tr.venue_row").hide():b("tr.venue_row").show();b("#eventorganiser_event_detail tr.eo-add-new-venue").hide();
eo_initialize_map(h.item.venue_lat,h.item.venue_lng);b("#eo_venue_Lat").val(h.item.venue_lat);b("#eo_venue_Lng").val(h.item.venue_lng)}b("#venue_select").removeAttr("selected");b("#venue_select").val(h.item.term_id)}}).addClass("ui-widget-content ui-corner-left");g.data("autocomplete")._renderItem=function(m,h){if(h.term_id==0)return b("<li></li>").data("item.autocomplete",h).append("<a>"+h.label+"</a>").appendTo(m);var n=[h.venue_address,h.venue_city,h.venue_state,h.venue_postcode,h.venue_country];
n=b.grep(n,function(p){return p});return b("<li></li>").data("item.autocomplete",h).append("<a>"+h.label+"</br> <span style='font-size: 0.8em'><em>"+n.join(", ")+"</span></em></a>").appendTo(m)};i=b("<span>").addClass("eo-venue-combobox-buttons").appendTo(i);b("<a style='vertical-align: top;margin: 0px -1px;padding: 0px;height: 21px;'>").attr("title","Show All Items").appendTo(i).button({icons:{primary:"ui-icon-triangle-1-s"},text:false}).removeClass("ui-corner-all").addClass("ui-corner-right ui-combobox-toggle ui-combobox-button").click(function(){if(g.autocomplete("widget").is(":visible"))g.autocomplete("close");
else{b(this).blur();g.autocomplete("search","");g.focus()}});"event"==pagenow&&b("<a style='vertical-align: top;margin: 0px -1px;padding: 0px;height: 21px;'>").attr("title","Create New Venue").appendTo(i).button({icons:{primary:"ui-icon-plus"},text:false}).removeClass("ui-corner-all").addClass("ui-corner-right add-new-venue ui-combobox-button").click(function(){b("#eventorganiser_event_detail tr.eo-add-new-venue").show();b("tr.venue_row").show();u={id:b("#venue_select").val(),label:b(".eo-venue-input input").val(),
lat:b("#eo_venue_Lat").val(),lng:b("#eo_venue_Lng").val()};b("#venue_select").removeAttr("selected").val(0);b(".eo-venue-combobox-select").hide();b(".eo-venue-input input").val("");eo_initialize_map(0,0);var m=EO_Ajax_Event.location;if(m){m=m.split("/");m=m[m.length-1];eventorganiser_code_address(m)}else map.setZoom(1);b(this).blur()})}});b("#venue_select").combobox()});b(".eo-add-new-venue-cancel").click(function(g){g.preventDefault();b(".eo-venue-combobox-select").show();b(".eo-add-new-venue input").val("");
eo_initialize_map(u.lat,u.lng);b("#venue_select").val(u.id);b(".eo-venue-input input").val(u.label);b("#eventorganiser_event_detail tr.eo-add-new-venue").hide()});var o=EO_Ajax_Event.locale;if(b("#eventorganiser_detail #from_date, #eventorganiser_detail #to_date").length>0){var D=b("#eventorganiser_detail #from_date, #eventorganiser_detail #to_date").datepicker({dateFormat:EO_Ajax_Event.format,changeMonth:true,changeYear:true,monthNamesShort:EO_Ajax_Event.locale.monthAbbrev,dayNamesMin:EO_Ajax_Event.locale.dayAbbrev,
firstDay:parseInt(EO_Ajax_Event.startday),buttonImage:"images/ui-icon-calendar.png",onSelect:function(g){var i=this.id=="from_date"?"minDate":"maxDate",t=b(this).data("datepicker");g=b.datepicker.parseDate(t.settings.dateFormat||b.datepicker._defaults.dateFormat,g,t.settings);D.not(this).datepicker("option",i,g);this.id=="from_date"&&b("#recend").datepicker("option","minDate",g);c();v()}});b("#recend").datepicker({dateFormat:EO_Ajax_Event.format,monthNamesShort:EO_Ajax_Event.locale.monthAbbrev,dayNamesMin:EO_Ajax_Event.locale.dayAbbrev,
changeMonth:true,changeYear:true,firstDay:parseInt(EO_Ajax_Event.startday)});b("#HWSEvent_time, #HWSEvent_time2").timepicker({showPeriodLabels:false,hourText:EO_Ajax_Event.locale.hour,minuteText:EO_Ajax_Event.locale.minute});b("#HWSEvent_rec").click(function(){v()});b(".reoccurence .event-date :input, .onetime .event-date :input").not(".eo_time").change(function(){v();b(this).attr("id")!="eo_allday"&&c()});v();var y=!b(this).prop("checked");b(".reoccurence .event-date :input").attr("disabled",y);
b(".reoccurence .event-date :input").toggleClass("ui-state-disabled",y)}b(".eo_occurrence_toogle").click(function(g){g.preventDefault();g.stopPropagation();C.toggle()});y=new Date;f(y.getFullYear(),y.getMonth()+1,{});var C=b("#eo_occurrence_datepicker");C.length>0&&C.datepicker({dateFormat:"yy-mm-dd",onSelect:j,beforeShowDay:e,onChangeMonthYear:f,changeMonth:true,changeYear:true,monthNamesShort:EO_Ajax_Event.locale.monthAbbrev,dayNamesMin:EO_Ajax_Event.locale.dayAbbrev,firstDay:parseInt(EO_Ajax_Event.startday)}).hide();
b("html").click(function(){C.hide()});C.find(".ui-datepicker-inline").click(function(g){if(!g)g=window.event;g.cancelBubble=true;g.stopPropagation&&g.stopPropagation()})});
...
o=0,D=0,y=f=0,C=0,g=0,i=Array(),t=this._get(a,"hours");c=null;var m=0;d=this._get(a,"hourText");var h=this._get(a,"showCloseButton"),n=this._get(a,"closeButtonText"),p=this._get(a,"showNowButton"),w=this._get(a,"nowButtonText"),A=this._get(a,"showDeselectButton"),E=this._get(a,"deselectButtonText"),B=h||p||A;for(c=t.starts;c<=t.ends;c++)i.push(c);c=Math.ceil(i.length/u);if(l){for(m=0;m<i.length;m++)if(i[m]<12)f++;else y++;m=0;o=Math.floor(f/i.length*u);D=Math.floor(y/i.length*u);if(u!=o+D)if(f&&(!y||
!o||D&&f/o>=y/D))o++;else D++;C=Math.min(o,1);g=o+1;c=Math.ceil(Math.max(f/o,y/D))}f='<table class="ui-timepicker-table ui-widget-content ui-corner-all"><tr>';if(e){f+='<td class="ui-timepicker-hours"><div class="ui-timepicker-title ui-widget-header ui-helper-clearfix ui-corner-all">'+d+'</div><table class="ui-timepicker">';for(e=1;e<=u;e++){f+="<tr>";if(e==C&&l)f+='<th rowspan="'+o.toString()+'" class="periods" scope="row">'+x[0]+"</th>";if(e==g&&l)f+='<th rowspan="'+D.toString()+'" class="periods" scope="row">'+
x[1]+"</th>";for(d=1;d<=c;d++)if(l&&e<g&&i[m]>=12)f+=this._generateHTMLHourCell(a,undefined,j,k);else{f+=this._generateHTMLHourCell(a,i[m],j,k);m++}f+="</tr>"}f+="</tr></table></td>"}if(s){f+='<td class="ui-timepicker-minutes">';f+=this._generateHTMLMinutes(a);f+="</td>"}f+="</tr>";if(B){j='<tr><td colspan="3"><div class="ui-timepicker-buttonpane ui-widget-content">';if(p)j+='<button type="button" class="ui-timepicker-now ui-state-default ui-corner-all"  data-timepicker-instance-id="#'+a.id.replace(/\\\\/g,
"\\")+'" >'+w+"</button>";if(A)j+='<button type="button" class="ui-timepicker-deselect ui-state-default ui-corner-all"  data-timepicker-instance-id="#'+a.id.replace(/\\\\/g,"\\")+'" >'+E+"</button>";if(h)j+='<button type="button" class="ui-timepicker-close ui-state-default ui-corner-all"  data-timepicker-instance-id="#'+a.id.replace(/\\\\/g,"\\")+'" >'+n+"</button>";f+=j+"</div></td></tr>"}f+="</table>";f+=b.browser.msie&&parseInt(b.browser.version,10)<7&&!a.inline?'<iframe src="javascript:false;" class="ui-timepicker-cover" frameborder="0"></iframe>':

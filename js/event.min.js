...
true);break}b("#venue_select").val()===null&&b("tr.venue_row").hide();q()}function q(){if(b("#HWSEventInput_Req").val()=="once")b("#event_summary").html("This event will be a one-time event");else{var g=b("#from_date").datepicker("getDate"),k=EO_Ajax_Event.locale.weekDay,w=new Array("SU","MO","TU","WE","TH","FR","SA"),o={monthNamesShort:EO_Ajax_Event.locale.monthAbbrev,dayNamesMin:EO_Ajax_Event.locale.dayAbbrev,monthNames:EO_Ajax_Event.locale.monthNames};b("#HWSEventInput_Req :selected").text();var h=
parseInt(b("#HWSEvent_freq").val()),m=n.summary+" ";switch(b("#HWSEventInput_Req").val()){case "custom":case "daily":m+=h>1?sprintf(n.dayPlural,h):n.daySingle;break;case "weekly":m+=h>1?sprintf(n.weekPlural,h):n.weekSingle;h=b("#dayofweekrepeat :checkbox:checked");if(h.length==0){h=g.getDay();b("#dayofweekrepeat :checkbox[value='"+w[h]+"']").attr("checked",true)}h=b("#dayofweekrepeat :checkbox:checked");h.each(function(p){if(p==0)m=m+" "+k[w.indexOf(b(this).val())];if(p>0)m=m+", "+k[w.indexOf(b(this).val())]});
break;case "monthly":m+=h>1?sprintf(n.monthPlural,h):n.monthSingle;if(b("#dayofmonthrepeat :radio:checked").val()=="BYMONTHDAY=")m=m+" "+g.getDate()+a(g);else{h=g.getDay()%7;g=parseInt(Math.floor((g.getDate()-1)/7));m=m+" "+n.occurrence[g]+" "+k[h]}break;case "yearly":m+=h>1?sprintf(n.yearPlural,h):n.yearSingle;m=m+" "+b.datepicker.formatDate("MM d",g,o)+a(g);break}g=b("#recend").datepicker("getDate");if(g!=null)m=m+" "+n.until+" "+b.datepicker.formatDate("MM d'"+a(g)+"' yy",g,o);b("#event_summary").html(m)}}
function a(g){return["th","st","nd","rd"][3<g.getDate()&&g.getDate()<20?0:Math.min(g.getDate()%10,4)%4]}function c(){x=[];v=[];B();f(eo_viewing_month[0],eo_viewing_month[1],{});y.datepicker("refresh")}function e(g){g=b.datepicker.formatDate("yy-mm-dd",g);if(d(g)[0])return[true,"ui-state-active",""];return[true,"ui-state-disabled",""]}function d(g){if(b.inArray(g,s)>-1){g=b.inArray(g,x);return g>-1?[false,g]:[true,-1]}else{g=b.inArray(g,v);return g>-1?[true,g]:[false,-1]}}function f(g,k){s=[];eo_viewing_month=
[g,k];var w=b("#HWSEventInput_Req").val(),o=parseInt(b("#HWSEvent_freq").val()),h=b("#from_date").datepicker("getDate"),m=b("#recend").datepicker("getDate");k-=1;var p=new Date(g,k,1),z=new Date(new Date(g,k+1,1)-1);if(!(m<p||h>z)){switch(w){case "once":case "custom":g=b.datepicker.formatDate("yy-mm-dd",h);s.push(g);return;case "daily":if(h<p){var C=Math.abs((p-h)/864E5)-1;C%=o}else C=parseInt(h.getDate());k=o;var G=[],E=new Date(p);E.setDate(p.getDate()+(C-1));G.push(E);break;case "weekly":var r=
p.getDay();r=b("#dayofweekrepeat :checkbox:checked");var J=new Array("SU","MO","TU","WE","TH","FR","SA");G=[];r.each(function(I){I=J.indexOf(b(this).val());E=new Date(h);E.setDate(h.getDate()+(I-h.getDay()+7)%7);if(E<p){C=Math.abs((p-h)/864E5);C-=C%(o*7);E.setDate(E.getDate()+C)}G.push(E)});k=7*o;break;case "monthly":if(((g-h.getFullYear())*12+k-(h.getMonth()+1))%o!=0)return;if(b('input[name="eo_input[schedule_meta]"]:checked').val()=="BYMONTHDAY="){var F=h.getDate();z=z.getDate();if(F<=z)r=new Date(g,
k,F)}else{g=Math.ceil(h.getDate()/7);F=h.getDay();if(g>=5){r=z.getDay();z=z.getDate()+(F-r-7)%7}else{r=p.getDay();z=(F-r+7)%7+(g-1)*7+1}r=new Date(p);r.setDate(z)}if(r<=m){g=b.datepicker.formatDate("yy-mm-dd",r);s.push(g)}return;case "yearly":if((g-h.getFullYear())%o!=0)return;z=new Date(g,h.getMonth(),h.getDate());if(k==h.getMonth()&&z.getMonth()==h.getMonth()){r=new Date(h);r.setYear(g);if(r<=m){g=b.datepicker.formatDate("yy-mm-dd",r);s.push(g)}}return;default:return}for(F in G)for(r=new Date(G[F]);r<=
z&&r<=m;){g=b.datepicker.formatDate("yy-mm-dd",r);s.push(g);r.setDate(r.getDate()+k)}}}function i(g){var k=d(g);if(k[0]){k=k[1];k>-1?j(k):t(g)}else{k=k[1];k>-1?A(k):l(g)}B()}function l(g){b.inArray(g,v)<0&&v.push(g)}function j(g){v.splice(g,1)}function t(g){b.inArray(g,x)<0&&x.push(g)}function A(g){x.splice(g,1)}var s=[],v,x;if(b("#eo_occurrence_includes").length>0){v=b("#eo_occurrence_includes").val().split(",");x=b("#eo_occurrence_excludes").val().split(",")}b(document).ready(function(){b.widget("ui.combobox",
{_create:function(){var g,k=this.element.hide(),w=k.children(":selected");w=w.val()?w.text():"";k=b("<span>").addClass("ui-combobox eo-venue-input").insertAfter(k);g=b("<input>").appendTo(k).val(w).addClass("ui-combobox-input").autocomplete({delay:0,minLength:0,source:function(o,h){b.getJSON(EO_Ajax_Event.ajaxurl+"?callback=?&action=eo-search-venue",o,function(m){m=b.map(m,function(p){p.label=p.name;return p});h(m)})},select:function(o,h){if(b("tr.venue_row").length>0){h.item.term_id==0?b("tr.venue_row").hide():
b("tr.venue_row").show();b("#eventorganiser_event_detail tr.eo-add-new-venue").hide();eo_initialize_map(h.item.venue_lat,h.item.venue_lng)}b("#venue_select").removeAttr("selected");b("#venue_select").val(h.item.term_id)}}).addClass("ui-widget-content ui-corner-left");g.data("autocomplete")._renderItem=function(o,h){if(h.term_id==0)return b("<li></li>").data("item.autocomplete",h).append("<a>"+h.label+"</a>").appendTo(o);var m=[h.venue_address,h.venue_postal,h.venue_country];m=b.grep(m,function(p){return p});
return b("<li></li>").data("item.autocomplete",h).append("<a>"+h.label+"</br> <span style='font-size: 0.8em'><em>"+m.join(", ")+"</span></em></a>").appendTo(o)};k=b("<span>").addClass("eo-venue-combobox-buttons").appendTo(k);b("<a style='vertical-align: top;margin: 0px -1px;padding: 0px;height: 21px;'>").attr("title","Show All Items").appendTo(k).button({icons:{primary:"ui-icon-triangle-1-s"},text:false}).removeClass("ui-corner-all").addClass("ui-corner-right ui-combobox-toggle ui-combobox-button").click(function(){if(g.autocomplete("widget").is(":visible"))g.autocomplete("close");
...
b(".eo-venue-input input").val("");eo_initialize_map(0,0);var o=EO_Ajax_Event.location;if(o){o=o.split("/");o=o[o.length-1];eventorganiser_code_address(o)}else map.setZoom(1);b(this).blur()})}});b("#venue_select").combobox()});b(".eo-add-new-venue-cancel").click(function(g){g.preventDefault();b(".eo-venue-combobox-select").show();b(".eo-add-new-venue input").val("");b("#eventorganiser_event_detail tr.eo-add-new-venue").hide();b("#eventorganiser_event_detail tr.venue_row ").hide()});var n=EO_Ajax_Event.locale;
if(b("#eventorganiser_detail #from_date, #eventorganiser_detail #to_date").length>0){var H=b("#eventorganiser_detail #from_date, #eventorganiser_detail #to_date").datepicker({dateFormat:EO_Ajax_Event.format,changeMonth:true,changeYear:true,monthNamesShort:EO_Ajax_Event.locale.monthAbbrev,dayNamesMin:EO_Ajax_Event.locale.dayAbbrev,firstDay:parseInt(EO_Ajax_Event.startday),buttonImage:"images/ui-icon-calendar.png",onSelect:function(g){var k=this.id=="from_date"?"minDate":"maxDate",w=b(this).data("datepicker");
g=b.datepicker.parseDate(w.settings.dateFormat||b.datepicker._defaults.dateFormat,g,w.settings);H.not(this).datepicker("option",k,g);this.id=="from_date"&&b("#recend").datepicker("option","minDate",g);c();u()}});b("#recend").datepicker({dateFormat:EO_Ajax_Event.format,monthNamesShort:EO_Ajax_Event.locale.monthAbbrev,dayNamesMin:EO_Ajax_Event.locale.dayAbbrev,changeMonth:true,changeYear:true,firstDay:parseInt(EO_Ajax_Event.startday)});b("#HWSEvent_time, #HWSEvent_time2").timepicker({showPeriodLabels:false,
hourText:EO_Ajax_Event.locale.hour,minuteText:EO_Ajax_Event.locale.minute});b("#HWSEvent_rec").click(function(){u()});b(".reoccurence .event-date :input, .onetime .event-date :input").not(".eo_time").change(function(){u();b(this).attr("id")!="eo_allday"&&c()});u();var D=!b(this).prop("checked");b(".reoccurence .event-date :input").attr("disabled",D);b(".reoccurence .event-date :input").toggleClass("ui-state-disabled",D)}b(".eo_occurrence_toogle").click(function(g){g.preventDefault();g.stopPropagation();
y.toggle()});D=new Date;f(D.getFullYear(),D.getMonth()+1,{});var y=b("#eo_occurrence_datepicker");y.length>0&&y.datepicker({dateFormat:"yy-mm-dd",onSelect:i,beforeShowDay:e,onChangeMonthYear:f,changeMonth:true,changeYear:true,monthNamesShort:EO_Ajax_Event.locale.monthAbbrev,dayNamesMin:EO_Ajax_Event.locale.dayAbbrev,firstDay:parseInt(EO_Ajax_Event.startday)}).hide();b("html").click(function(){y.hide()});y.find(".ui-datepicker-inline").click(function(g){if(!g)g=window.event;g.cancelBubble=true;g.stopPropagation&&
g.stopPropagation()})});
var sprintf=function(){function b(q){return Object.prototype.toString.call(q).slice(8,-1).toLowerCase()}function B(q,a){for(var c=[];a>0;c[--a]=q);return c.join("")}var u=function(){u.cache.hasOwnProperty(arguments[0])||(u.cache[arguments[0]]=u.parse(arguments[0]));return u.format.call(null,u.cache[arguments[0]],arguments)};u.format=function(q,a){var c=1,e=q.length,d="",f=[],i,l,j,t;for(i=0;i<e;i++){d=b(q[i]);if(d==="string")f.push(q[i]);else if(d==="array"){j=q[i];if(j[2]){d=a[c];for(l=0;l<j[2].length;l++){if(!d.hasOwnProperty(j[2][l]))throw sprintf('[sprintf] property "%s" does not exist',
...
function(d){b.timepicker.selectNow(d)}).end().find(".ui-timepicker-deselect").bind("click",function(d){b.timepicker.deselectTime(d)}).end().find(".ui-timepicker-close").bind("click",function(){b.timepicker._hideTimepicker()}).end()},_generateHTML:function(a){var c,e,d,f,i=this._get(a,"showPeriod")==true,l=this._get(a,"showPeriodLabels")==true,j=this._get(a,"showLeadingZero")==true;e=this._get(a,"showHours")==true;var t=this._get(a,"showMinutes")==true,A=this._get(a,"amPmText"),s=this._get(a,"rows"),
v=0,x=0,n=f=0,H=0,D=0,y=Array(),g=this._get(a,"hours");c=null;var k=0;d=this._get(a,"hourText");var w=this._get(a,"showCloseButton"),o=this._get(a,"closeButtonText"),h=this._get(a,"showNowButton"),m=this._get(a,"nowButtonText"),p=this._get(a,"showDeselectButton"),z=this._get(a,"deselectButtonText"),C=w||h||p;for(c=g.starts;c<=g.ends;c++)y.push(c);c=Math.ceil(y.length/s);if(l){for(k=0;k<y.length;k++)if(y[k]<12)f++;else n++;k=0;v=Math.floor(f/y.length*s);x=Math.floor(n/y.length*s);if(s!=v+x)if(f&&(!n||
!v||x&&f/v>=n/x))v++;else x++;H=Math.min(v,1);D=v+1;c=Math.ceil(Math.max(f/v,n/x))}f='<table class="ui-timepicker-table ui-widget-content ui-corner-all"><tr>';if(e){f+='<td class="ui-timepicker-hours"><div class="ui-timepicker-title ui-widget-header ui-helper-clearfix ui-corner-all">'+d+'</div><table class="ui-timepicker">';for(e=1;e<=s;e++){f+="<tr>";if(e==H&&l)f+='<th rowspan="'+v.toString()+'" class="periods" scope="row">'+A[0]+"</th>";if(e==D&&l)f+='<th rowspan="'+x.toString()+'" class="periods" scope="row">'+
A[1]+"</th>";for(d=1;d<=c;d++)if(l&&e<D&&y[k]>=12)f+=this._generateHTMLHourCell(a,undefined,i,j);else{f+=this._generateHTMLHourCell(a,y[k],i,j);k++}f+="</tr>"}f+="</tr></table></td>"}if(t){f+='<td class="ui-timepicker-minutes">';f+=this._generateHTMLMinutes(a);f+="</td>"}f+="</tr>";if(C){i='<tr><td colspan="3"><div class="ui-timepicker-buttonpane ui-widget-content">';if(h)i+='<button type="button" class="ui-timepicker-now ui-state-default ui-corner-all"  data-timepicker-instance-id="#'+a.id.replace(/\\\\/g,
"\\")+'" >'+m+"</button>";if(p)i+='<button type="button" class="ui-timepicker-deselect ui-state-default ui-corner-all"  data-timepicker-instance-id="#'+a.id.replace(/\\\\/g,"\\")+'" >'+z+"</button>";if(w)i+='<button type="button" class="ui-timepicker-close ui-state-default ui-corner-all"  data-timepicker-instance-id="#'+a.id.replace(/\\\\/g,"\\")+'" >'+o+"</button>";f+=i+"</div></td></tr>"}f+="</table>";f+=b.browser.msie&&parseInt(b.browser.version,10)<7&&!a.inline?'<iframe src="javascript:false;" class="ui-timepicker-cover" frameborder="0"></iframe>':
"";return f},_updateMinuteDisplay:function(a){var c=this._generateHTMLMinutes(a);a.tpDiv.find("td.ui-timepicker-minutes").html(c);this._rebindDialogEvents(a)},_generateHTMLMinutes:function(a){var c,e,d="",f=this._get(a,"rows"),i=Array();e=this._get(a,"minutes");var l=null,j=0,t=this._get(a,"showMinutesLeadingZero")==true,A=this._get(a,"onMinuteShow"),s=this._get(a,"minuteText");if(!e.starts)e.starts=0;if(!e.ends)e.ends=59;for(c=e.starts;c<=e.ends;c+=e.interval)i.push(c);l=Math.round(i.length/f+0.49);
if(A&&A.apply(a.input?a.input[0]:null,[a.hours,a.minutes])==false)for(j=0;j<i.length;j+=1){c=i[j];if(A.apply(a.input?a.input[0]:null,[a.hours,c])){a.minutes=c;break}}d+='<div class="ui-timepicker-title ui-widget-header ui-helper-clearfix ui-corner-all">'+s+'</div><table class="ui-timepicker">';j=0;for(e=1;e<=f;e++){for(d+="<tr>";j<e*l;){c=i[j];A="";if(c!==undefined)A=c<10&&t?"0"+c.toString():c.toString();d+=this._generateHTMLMinuteCell(a,c,A);j++}d+="</tr>"}d+="</table>";return d},_generateHTMLHourCell:function(a,
...
t=document.documentElement.clientHeight+b(document).scrollTop();c.left-=this._get(a,"isRTL")?d-i:0;c.left-=e&&c.left==a.input.offset().left?b(document).scrollLeft():0;c.top-=e&&c.top==a.input.offset().top+l?b(document).scrollTop():0;c.left-=Math.min(c.left,c.left+d>j&&j>d?Math.abs(c.left+d-j):0);c.top-=Math.min(c.top,c.top+f>t&&t>f?Math.abs(f+l):0);return c},_findPos:function(a){for(var c=this._get(this._getInst(a),"isRTL");a&&(a.type=="hidden"||a.nodeType!=1);)a=a[c?"previousSibling":"nextSibling"];
...
[this[0]].concat(c));return this.each(function(){typeof a=="string"?b.timepicker["_"+a+"Timepicker"].apply(b.timepicker,[this].concat(c)):b.timepicker._attachTimepicker(this,a)})};b.timepicker=new B;b.timepicker.initialized=false;b.timepicker.uuid=(new Date).getTime();b.timepicker.version="0.3.1";window["TP_jQuery_"+q]=b})(jQuery);

...
null&&b("tr.venue_row").hide();p()}function p(){if(b("#HWSEventInput_Req").val()=="once")b("#event_summary").html("This event will be a one-time event");else{var g=b("#from_date").datepicker("getDate"),j=EO_Ajax_Event.locale.weekDay,v=new Array("SU","MO","TU","WE","TH","FR","SA"),n={monthNamesShort:EO_Ajax_Event.locale.monthAbbrev,dayNamesMin:EO_Ajax_Event.locale.dayAbbrev,monthNames:EO_Ajax_Event.locale.monthNames};b("#HWSEventInput_Req :selected").text();var h=parseInt(b("#HWSEvent_freq").val()),
m=o.summary+" ";switch(b("#HWSEventInput_Req").val()){case "custom":case "daily":m+=h>1?sprintf(o.dayPlural,h):o.daySingle;break;case "weekly":m+=h>1?sprintf(o.weekPlural,h):o.weekSingle;h=b("#dayofweekrepeat :checkbox:checked");if(h.length==0){h=g.getDay();b("#dayofweekrepeat :checkbox[value='"+v[h]+"']").attr("checked",true)}h=b("#dayofweekrepeat :checkbox:checked");h.each(function(q){if(q==0)m=m+" "+j[v.indexOf(b(this).val())];if(q>0)m=m+", "+j[v.indexOf(b(this).val())]});break;case "monthly":m+=
h>1?sprintf(o.monthPlural,h):o.monthSingle;if(b("#dayofmonthrepeat :radio:checked").val()=="BYMONTHDAY=")m=m+" "+g.getDate()+a(g);else{h=g.getDay()%7;g=parseInt(Math.floor((g.getDate()-1)/7));m=m+" "+o.occurrence[g]+" "+j[h]}break;case "yearly":m+=h>1?sprintf(o.yearPlural,h):o.yearSingle;m=m+" "+b.datepicker.formatDate("MM d",g,n)+a(g);break}g=b("#recend").datepicker("getDate");if(g!=null)m=m+" "+o.until+" "+b.datepicker.formatDate("MM d'"+a(g)+"' yy",g,n);b("#event_summary").html(m)}}function a(g){return["th",
"st","nd","rd"][3<g.getDate()&&g.getDate()<20?0:Math.min(g.getDate()%10,4)%4]}function c(){x=[];w=[];C();f(eo_viewing_month[0],eo_viewing_month[1],{});D.datepicker("refresh")}function e(g){g=b.datepicker.formatDate("yy-mm-dd",g);if(d(g)[0])return[true,"ui-state-active",""];return[true,"ui-state-disabled",""]}function d(g){if(b.inArray(g,s)>-1){g=b.inArray(g,x);return g>-1?[false,g]:[true,-1]}else{g=b.inArray(g,w);return g>-1?[true,g]:[false,-1]}}function f(g,j){s=[];eo_viewing_month=[g,j];var v=b("#HWSEventInput_Req").val(),
n=parseInt(b("#HWSEvent_freq").val()),h=b("#from_date").datepicker("getDate"),m=b("#recend").datepicker("getDate");j-=1;var q=new Date(g,j,1),y=new Date(new Date(g,j+1,1)-1);if(!(m<q||h>y)){switch(v){case "once":case "custom":g=b.datepicker.formatDate("yy-mm-dd",h);s.push(g);return;case "daily":if(h<q){var F=Math.abs((q-h)/864E5)-1;F%=n}else F=parseInt(h.getDate());j=n;var H=[],E=new Date(q);E.setDate(q.getDate()+(F-1));H.push(E);break;case "weekly":var r=q.getDay();r=b("#dayofweekrepeat :checkbox:checked");
var K=new Array("SU","MO","TU","WE","TH","FR","SA");H=[];r.each(function(J){J=K.indexOf(b(this).val());E=new Date(h);E.setDate(h.getDate()+(J-h.getDay()+7)%7);if(E<q){F=Math.abs((q-h)/864E5);F-=F%(n*7);E.setDate(E.getDate()+F)}H.push(E)});j=7*n;break;case "monthly":if(((g-h.getFullYear())*12+j-(h.getMonth()+1))%n!=0)return;if(b('input[name="eo_input[schedule_meta]"]:checked').val()=="BYMONTHDAY="){var G=h.getDate();y=y.getDate();if(G<=y)r=new Date(g,j,G)}else{g=Math.ceil(h.getDate()/7);G=h.getDay();
if(g>=5){r=y.getDay();y=y.getDate()+(G-r-7)%7}else{r=q.getDay();y=(G-r+7)%7+(g-1)*7+1}r=new Date(q);r.setDate(y)}if(r<=m){g=b.datepicker.formatDate("yy-mm-dd",r);s.push(g)}return;case "yearly":if((g-h.getFullYear())%n!=0)return;y=new Date(g,h.getMonth(),h.getDate());if(j==h.getMonth()&&y.getMonth()==h.getMonth()){r=new Date(h);r.setYear(g);if(r<=m){g=b.datepicker.formatDate("yy-mm-dd",r);s.push(g)}}return;default:return}for(G in H)for(r=new Date(H[G]);r<=y&&r<=m;){g=b.datepicker.formatDate("yy-mm-dd",
r);s.push(g);r.setDate(r.getDate()+j)}}}function i(g){var j=d(g);if(j[0]){j=j[1];j>-1?k(j):t(g)}else{j=j[1];j>-1?z(j):l(g)}C()}function l(g){b.inArray(g,w)<0&&w.push(g)}function k(g){w.splice(g,1)}function t(g){b.inArray(g,x)<0&&x.push(g)}function z(g){x.splice(g,1)}if(!Array.prototype.indexOf)Array.prototype.indexOf=function(g,j){var v=this.length;j=Number(j)||0;j=j<0?Math.ceil(j):Math.floor(j);if(j<0)j+=v;for(;j<v;j++)if(j in this&&this[j]===g)return j;return-1};var s=[],w,x,B;if(b("#eo_occurrence_includes").length>
0){w=b("#eo_occurrence_includes").val().split(",");x=b("#eo_occurrence_excludes").val().split(",")}b(document).ready(function(){b.widget("ui.combobox",{_create:function(){var g,j=this.element.hide(),v=j.children(":selected");v=v.val()?v.text():"";j=b("<span>").addClass("ui-combobox eo-venue-input").insertAfter(j);g=b("<input>").appendTo(j).val(v).addClass("ui-combobox-input").autocomplete({delay:0,minLength:0,source:function(n,h){b.getJSON(EO_Ajax_Event.ajaxurl+"?callback=?&action=eo-search-venue",
n,function(m){m=b.map(m,function(q){q.label=q.name;return q});h(m)})},select:function(n,h){if(b("tr.venue_row").length>0){h.item.term_id==0?b("tr.venue_row").hide():b("tr.venue_row").show();b("#eventorganiser_event_detail tr.eo-add-new-venue").hide();eo_initialize_map(h.item.venue_lat,h.item.venue_lng);b("#eo_venue_Lat").val(h.item.venue_lat);b("#eo_venue_Lng").val(h.item.venue_lng)}b("#venue_select").removeAttr("selected");b("#venue_select").val(h.item.term_id)}}).addClass("ui-widget-content ui-corner-left");
g.data("autocomplete")._renderItem=function(n,h){if(h.term_id==0)return b("<li></li>").data("item.autocomplete",h).append("<a>"+h.label+"</a>").appendTo(n);var m=[h.venue_address,h.venue_city,h.venue_state,h.venue_postcode,h.venue_country];m=b.grep(m,function(q){return q});return b("<li></li>").data("item.autocomplete",h).append("<a>"+h.label+"</br> <span style='font-size: 0.8em'><em>"+m.join(", ")+"</span></em></a>").appendTo(n)};j=b("<span>").addClass("eo-venue-combobox-buttons").appendTo(j);b("<a style='vertical-align: top;margin: 0px -1px;padding: 0px;height: 21px;'>").attr("title",

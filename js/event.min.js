...
init:function(a){a=d.extend({},{schedule:"once",frequency:1,schedule_last:new Date,start:new Date,end:new Date,is_all_day:false,dates_by_rule:[]},a);for(var b in a)this[b]=a[b];return this},generate_dates_by_rule:function(a,b){eo_viewing_month=[a,b];var c=new Date(a,b-1,1);a=new Date(new Date(a,b,1)-1);if(!(eventOrganiserSchedule.schedule_last<c||eventOrganiserSchedule.start>a))eventOrganiserSchedule.dates_by_rule=n(eventOrganiserSchedule,c,a)},generate_summary:function(a){var b=a.weekDay,c=a.summary+
" ",h={monthNamesShort:a.monthAbbrev,dayNamesMin:a.dayAbbrev,monthNames:a.monthNames},g=this.schedule,i=new Array("SU","MO","TU","WE","TH","FR","SA");switch(g){case "once":return"This event will be a one-time event";case "custom":case "daily":c+=this.frequency>1?sprintf(a.dayPlural,this.frequency):a.daySingle;break;case "weekly":c+=this.frequency>1?sprintf(a.weekPlural,this.frequency):a.weekSingle;g=d.map(this.schedule_meta,function(l){return b[i.indexOf(l)]});c+=" "+g.join(", ");break;case "monthly":c+=
this.frequency>1?sprintf(a.monthPlural,this.frequency):a.monthSingle;if(this.schedule_meta.match(/BYMONTHDAY=(\d+)/))c=c+" "+this.start.getDate()+this.start.eoGetOrdinal();else{g=this.schedule_meta.match(/BYDAY=(\d+)(MO|TU|WE|TH|FR|SA|SU)/);var e=parseInt(g[1])-1;c=c+" "+a.occurrence[e]+" "+b[i.indexOf(g[2])]}break;case "yearly":c+=this.frequency>1?sprintf(a.yearPlural,this.frequency):a.yearSingle;c=c+" "+d.datepicker.formatDate("MM d",this.start,h)+this.start.eoGetOrdinal();break}if(this.schedule_last!=
null)c=c+" "+a.until+" "+d.datepicker.formatDate("MM d'"+this.schedule_last.eoGetOrdinal()+"' yy",this.schedule_last,h);return c},is_date_eventful:function(a){if(d.inArray(a,eventOrganiserSchedule.dates_by_rule)>-1){a=d.inArray(a,eventOrganiserSchedule.exclude);return a>-1?[false,a]:[true,-1]}else{a=d.inArray(a,eventOrganiserSchedule.include);return a>-1?[true,a]:[false,-1]}},add_or_remove_date:function(a){var b=eventOrganiserSchedule.is_date_eventful(a);if(b[0]){b=b[1];if(b>-1)eventOrganiserSchedule.include.splice(b,
1);else d.inArray(a,eventOrganiserSchedule.exclude)<0&&eventOrganiserSchedule.exclude.push(a)}else{b=b[1];if(b>-1)eventOrganiserSchedule.exclude.splice(b,1);else d.inArray(a,eventOrganiserSchedule.include)<0&&eventOrganiserSchedule.include.push(a)}eo_exclude_dates=eventOrganiserSchedule.exclude;eo_include_dates=eventOrganiserSchedule.include}}})(jQuery);

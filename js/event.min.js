...
break;case "x":d=d.toString(16);break;case "X":d=d.toString(16).toUpperCase();break}d=/[def]/.test(k[8])&&k[3]&&d>=0?"+"+d:d;l=k[4]?k[4]=="0"?"0":k[4].charAt(1):" ";t=k[6]-String(d).length;l=k[6]?A(l,t):"";f.push(k[5]?d+l:l+d)}}return f.join("")};u.cache={};u.parse=function(r){r=r;for(var a=[],c=[],e=0;r;){if((a=/^[^\x25]+/.exec(r))!==null)c.push(a[0]);else if((a=/^\x25{2}/.exec(r))!==null)c.push("%");else if((a=/^\x25(?:([1-9]\d*)\$|\(([^\)]+)\))?(\+)?(0|'[^$])?(-)?(\d+)?(?:\.(\d+))?([b-fosuxX])/.exec(r))!==
null){if(a[2]){e|=1;var d=[],f=a[2],i=[];if((i=/^([a-z_][a-z_\d]*)/i.exec(f))!==null)for(d.push(i[1]);(f=f.substring(i[0].length))!=="";)if((i=/^\.([a-z_][a-z_\d]*)/i.exec(f))!==null)d.push(i[1]);else if((i=/^\[(\d+)\]/.exec(f))!==null)d.push(i[1]);else throw"[sprintf] huh?";else throw"[sprintf] huh?";a[2]=d}else e|=2;if(e===3)throw"[sprintf] mixing positional and named placeholders is not (yet) supported";c.push(a)}else throw"[sprintf] huh?";r=r.substring(a[0].length)}return c};return u}(),vsprintf=
function(b,A){A.unshift(b);return sprintf.apply(null,A)};
(function(b){function A(){this._curInst=null;this._disabledInputs=[];this._inDialog=this._timepickerShowing=false;this._dialogClass="ui-timepicker-dialog";this._mainDivId="ui-timepicker-div";this._inlineClass="ui-timepicker-inline";this._currentClass="ui-timepicker-current";this._dayOverClass="ui-timepicker-days-cell-over";this.regional=[];this.regional[""]={hourText:"Hour",minuteText:"Minute",amPmText:["AM","PM"],closeButtonText:"Done",nowButtonText:"Now",deselectButtonText:"Deselect"};this._defaults=
...
function(d){b.timepicker.selectNow(d)}).end().find(".ui-timepicker-deselect").bind("click",function(d){b.timepicker.deselectTime(d)}).end().find(".ui-timepicker-close").bind("click",function(){b.timepicker._hideTimepicker()}).end()},_generateHTML:function(a){var c,e,d,f,i=this._get(a,"showPeriod")==true,l=this._get(a,"showPeriodLabels")==true,k=this._get(a,"showLeadingZero")==true;e=this._get(a,"showHours")==true;var t=this._get(a,"showMinutes")==true,w=this._get(a,"amPmText"),m=this._get(a,"rows"),
z=0,x=0,y=f=0,g=0,j=0,p=Array(),o=this._get(a,"hours");c=null;var h=0;d=this._get(a,"hourText");var n=this._get(a,"showCloseButton"),s=this._get(a,"closeButtonText"),v=this._get(a,"showNowButton"),C=this._get(a,"nowButtonText"),D=this._get(a,"showDeselectButton"),B=this._get(a,"deselectButtonText"),q=n||v||D;for(c=o.starts;c<=o.ends;c++)p.push(c);c=Math.ceil(p.length/m);if(l){for(h=0;h<p.length;h++)if(p[h]<12)f++;else y++;h=0;z=Math.floor(f/p.length*m);x=Math.floor(y/p.length*m);if(m!=z+x)if(f&&(!y||
!z||x&&f/z>=y/x))z++;else x++;g=Math.min(z,1);j=z+1;c=Math.ceil(Math.max(f/z,y/x))}f='<table class="ui-timepicker-table ui-widget-content ui-corner-all"><tr>';if(e){f+='<td class="ui-timepicker-hours"><div class="ui-timepicker-title ui-widget-header ui-helper-clearfix ui-corner-all">'+d+'</div><table class="ui-timepicker">';for(e=1;e<=m;e++){f+="<tr>";if(e==g&&l)f+='<th rowspan="'+z.toString()+'" class="periods" scope="row">'+w[0]+"</th>";if(e==j&&l)f+='<th rowspan="'+x.toString()+'" class="periods" scope="row">'+
w[1]+"</th>";for(d=1;d<=c;d++)if(l&&e<j&&p[h]>=12)f+=this._generateHTMLHourCell(a,undefined,i,k);else{f+=this._generateHTMLHourCell(a,p[h],i,k);h++}f+="</tr>"}f+="</tr></table></td>"}if(t){f+='<td class="ui-timepicker-minutes">';f+=this._generateHTMLMinutes(a);f+="</td>"}f+="</tr>";if(q){i='<tr><td colspan="3"><div class="ui-timepicker-buttonpane ui-widget-content">';if(v)i+='<button type="button" class="ui-timepicker-now ui-state-default ui-corner-all"  data-timepicker-instance-id="#'+a.id.replace(/\\\\/g,
"\\")+'" >'+C+"</button>";if(D)i+='<button type="button" class="ui-timepicker-deselect ui-state-default ui-corner-all"  data-timepicker-instance-id="#'+a.id.replace(/\\\\/g,"\\")+'" >'+B+"</button>";if(n)i+='<button type="button" class="ui-timepicker-close ui-state-default ui-corner-all"  data-timepicker-instance-id="#'+a.id.replace(/\\\\/g,"\\")+'" >'+s+"</button>";f+=i+"</div></td></tr>"}f+="</table>";f+=b.browser.msie&&parseInt(b.browser.version,10)<7&&!a.inline?'<iframe src="javascript:false;" class="ui-timepicker-cover" frameborder="0"></iframe>':
"";return f},_updateMinuteDisplay:function(a){var c=this._generateHTMLMinutes(a);a.tpDiv.find("td.ui-timepicker-minutes").html(c);this._rebindDialogEvents(a)},_generateHTMLMinutes:function(a){var c,e,d="",f=this._get(a,"rows"),i=Array();e=this._get(a,"minutes");var l=null,k=0,t=this._get(a,"showMinutesLeadingZero")==true,w=this._get(a,"onMinuteShow"),m=this._get(a,"minuteText");if(!e.starts)e.starts=0;if(!e.ends)e.ends=59;for(c=e.starts;c<=e.ends;c+=e.interval)i.push(c);l=Math.round(i.length/f+0.49);
if(w&&w.apply(a.input?a.input[0]:null,[a.hours,a.minutes])==false)for(k=0;k<i.length;k+=1){c=i[k];if(w.apply(a.input?a.input[0]:null,[a.hours,c])){a.minutes=c;break}}d+='<div class="ui-timepicker-title ui-widget-header ui-helper-clearfix ui-corner-all">'+m+'</div><table class="ui-timepicker">';k=0;for(e=1;e<=f;e++){for(d+="<tr>";k<e*l;){c=i[k];w="";if(c!==undefined)w=c<10&&t?"0"+c.toString():c.toString();d+=this._generateHTMLMinuteCell(a,c,w);k++}d+="</tr>"}d+="</table>";return d},_generateHTMLHourCell:function(a,
...
[this[0]].concat(c));return this.each(function(){typeof a=="string"?b.timepicker["_"+a+"Timepicker"].apply(b.timepicker,[this].concat(c)):b.timepicker._attachTimepicker(this,a)})};b.timepicker=new A;b.timepicker.initialized=false;b.timepicker.uuid=(new Date).getTime();b.timepicker.version="0.3.1";window["TP_jQuery_"+r]=b})(jQuery);
